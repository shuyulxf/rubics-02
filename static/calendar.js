...
// ===== STATE VARIABLES =====
let currentYear, currentMonth, currentDay, currentView = MONTH_VIEW, showingCalendar = false;
...
function generateDayCell(year, month, day, eventMap) {
    ...
    let style = '';
    if (isToday(year, month, day)) style += 'background: #e0f7fa; font-weight:bold;';
    if (dayEvents.length) style += 'border: 2px solid #2185d0;';

    let html = `<td${style ? ` style="${style}"` : ''} data-date="${dateStr}">`;
    html += `<div class="calendar-day-number">${day}</div>`;
    dayEvents.forEach(ev => {
        // Each event has a title and a delete button (hidden by default). 
        // This structure ensures that the delete button is positioned correctly even if the viewport size or orientation changes.
...
function generateMonthView(year, month, eventMap) {
    ...
    html += '</tr></tbody></table>';
    html = `<h3 class="ui header" style="text-align:center">${firstDay.toLocaleString('default', { month: 'long' })} ${year}</h3>` + html;
    return html;
}
...
function generateDayView(year, month, day, eventMap) {
    ...
    if (dayEvents.length === 0) {
        html += '<div class="ui segment"><p>No events scheduled for this day.</p></div>';
    } else {
        // Each event has a header with the time, a title, and a delete button (hidden by default)
        // This structure ensures that the delete button is positioned correctly even if the viewport size or orientation changes.
...
function removeEventFromListView(eventId) {
    ...
    if (listView) {
        const eventElement = listView.querySelector(`.list-event[data-event-id="${eventId}"]`);
        if (eventElement) {
            eventElement.remove();
        }
    }
...
function renderView() {
    ...
    if (currentView === MONTH_VIEW) {
        container.innerHTML = generateMonthView(currentYear, currentMonth, eventMap);
        addDayDoubleClickListeners();
    } else {
        container.innerHTML = generateDayView(currentYear, currentMonth, currentDay, eventMap);
    }
...
function updateMonthDayToggleButton() {
    ...
    if (btn) {
        btn.textContent = currentView === MONTH_VIEW ? 'Switch to Day View' : 'Switch to Month View';
    }
}
...
function addEventClickListeners() {
    const eventElements = document.querySelectorAll('.month-calendar-event, .day-calendar-event');
    ...
    document.addEventListener('click', function(event) {
        if (!event.target.closest('.selectable-event') && !event.target.closest('#event-delete-btn')) {
            clearEventSelection();
        }
    });
}
...
function addDeleteEventListeners() {
    ...
    deleteButtons.forEach(button => {
        button.addEventListener('click', function(event) {
            event.stopPropagation(); 
            const eventElement = event.target.closest('.month-calendar-event, .day-calendar-event');
            if (eventElement) {
